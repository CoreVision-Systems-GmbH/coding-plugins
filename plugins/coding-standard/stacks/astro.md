...
## 3. Werkzeugkette
- Befehle aus dem Repo ermitteln (`CLAUDE.md`, `package.json`). Standard für neue Sites: `astro check` (Typen und Vorlagen), `astro build`, `node --test` über `tests/**/*.test.mjs` (kein Test-Framework), `npm audit --omit=dev --audit-level=high`.
- Abhängigkeiten mit Caret in `package.json`, festgezurrt durch `package-lock.json` (`npm ci`); Bumps als eigener PR (Dependabot `npm`).
- CI-Reihenfolge: `npm ci` → `npm run check` → `npm run build` → `npm test` → `npm audit`. Der Bau ist die eigentliche Prüfung: Er scheitert bei Schemaverstößen in Collections, fehlenden Bildern und fehlender `site`.
...
## 4. Betriebsvertrag
1. **Runtime:** Bau in `node:24-alpine`, Laufzeit `caddy:2-alpine` mit `file_server` aus `/srv` auf 8080. Der Edge-Caddy des Hosts terminiert TLS und verteilt nach Hostname; kein veröffentlichter Port, Container nur im Netz `edge`.
